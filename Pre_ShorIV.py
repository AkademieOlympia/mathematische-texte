...
def N_spectral_profile(N: int,
    ...
    PN = build_full_transition_matrix_refined_v3(
        sqrt_s=sqrt_s,
        qparams=qparams,
        eparams=eparams,
        rparams=rparams,
...
def profile_vector_v3(profile: dict, H_inf: float = 1.0) -> np.ndarray:
    abs_part = [
        profile["absorption"]["A"],
        profile["absorption"]["B"],
        profile["absorption"]["C"],
    ...
    # Hurwitzdruck-abgeleitete Merkmale
    hz = H_inf * (
    ...
    return np.array(abs_part + qsd_part + eig_part + [hz, h0, hs], dtype=float)


def build_profile_matrix(profiles: list, H_inf: float = 1.0):
    Ns = [p["N"] for p in profiles]
    X = np.vstack([profile_vector_v3(p, H_inf=H_inf) for p in profiles])
    return Ns, X


def build_labeled_profile_matrix(profiles, labels_map, H_inf=1.0):
    rows = []
    ys = []
    ns = []
...
def train_and_report_classifier(profiles, labels_map, H_inf=1.0):
    ...
    for i, N in enumerate(Ns):
        ...
        print(f"N={N:>4d}  true={y[i]:>10s}  pred={pred:>10s}  dist={dist:.4f}  {ok}")
...
def predict_number_family(N: int, sqrt_s: float,
    ...
    pred, dist = classify_by_nearest_centroid(x_scaled[0], trained["centroids"])
    return profile, pred, dist
...
def feature_names_for_profiles(X_shape_1: int):
    ...
    # 3 Hurwitz-Merkmale am Ende abziehen
    n_hurwitz = 3
    n_eigs = X_shape_1 - len(base) - n_hurwitz
    eigs = [f"eig_{idx+1}" for idx in range(n_eigs)]
    hurwitz = ["hurwitz_z", "hurwitz_0", "hurwitz_steps"]
    return base + eigs + hurwitz
...
TRAIN_LABELS = {
    13: "prime-like", 17: "prime-like", 19: "prime-like", 23: "prime-like",
    29: "prime-like", 31: "prime-like", 37: "prime-like", 41: "prime-like", 43: "prime-like",
...
if __name__ == "__main__":
    hparams = HurwitzPressureParams(
        H_inf=20.02  # <-- hier deinen echten Wert einsetzen
    )
    ...
    extended_profiles = compare_numbers(extended_numbers, sqrt_s=3.5, hparams=hparams)
    ...
    # Beispiel-Vorhersagen für neue Zahlen
    for N_test in [27, 45, 63, 105, 143]:
