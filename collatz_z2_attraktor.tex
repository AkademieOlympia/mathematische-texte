...
\begin{tcolorbox}[heuristik,title={Noch offen (kein \texttt{sorry}, aber Platzhalter/TODO)}]
\begin{itemize}[nosep]
  \item \texttt{highComplexity}, \texttt{ExceptionSetApprox}: echte $E_K$ aus Collatz-Bahnen;
  \item \texttt{exitsExceptionApprox}: Iteration von \texttt{collatzU} (\texttt{Function.iterate});
  \item \texttt{inTrivialAttraktorApprox}: formal als $\|x-1\|_2<1$ auf $\Ztwo$;
  \item Limes-Definition von $E$ (Definition~\ref{def:E}) als abgeschlossene H\"ulle in $\Ztwo$;
