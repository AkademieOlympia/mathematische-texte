...
\subsection{Bernoulli-Uhr auf einer chiralen Doppelellipse}
...
z_0=-1-\frac{\mathrm{i}}{12}
\]
der feste Referenzpunkt und
\[
r_m=2^{-m},\qquad \theta_m=\frac{\pi}{2}m.
\]
Für $m\ge 1$ definieren wir die Bernoulli-Uhrzelle
\[
\mathcal{U}(m)
=
\Big(
...
\Big).
\]
Dabei bezeichnen $B_{2m}$ die geraden Bernoulli-Zahlen, welche durch
\[
B_{2m}=-2m\,\zeta(1-2m)
\]
gegeben sind.
Die Folge $\theta_m=\frac{\pi}{2}m$ entspricht einem Dreierschritt auf dem
Zwölferkreis, da
\[
3\cdot\frac{2\pi}{12}=\frac{\pi}{2}.
\]
Die drei Komponenten von $\mathcal{U}(m)$ werden als linke, zentrale und
rechte Bernoulli-Stufe interpretiert.
\end{definition}

\begin{remark}[Chirale Interpretation]
Die Konstruktion besitzt eine natürliche Verbindung zur EABC-Struktur.
Der Vierteltakt
$E\to A\to B\to C\to E$
wird durch den Winkel $\theta_m=\frac{\pi}{2}m$ realisiert.
Die beiden zyklischen Wörter $\mathrm{ABCE}$ und $\mathrm{CEAB}$
unterscheiden sich um zwei Schritte auf dem Viererzyklus und damit um die
Phase $\pi$.
Sie können daher als chiral konjugierte Umläufe derselben geometrischen
Struktur interpretiert werden.
Weiter gilt
\[
z_0=\zeta(-1)(12+\mathrm{i}),
\]
wodurch die Bernoulli-Zahlen, die Zetafunktion und die Zwölferstruktur in
einem gemeinsamen Referenzpunkt zusammenlaufen.
Die Bernoulli-Uhr liefert somit eine geometrische Organisation der Größen
$\{B_{2m}\}_{m\ge 1}$ auf einer durch EABC motivierten Doppelellipse.
\end{remark}

\begin{remark}[Nichtbehauptung]
Die Bernoulli-Uhr ist eine geometrische Hilfskonstruktion.
Insbesondere wird nicht behauptet,
\begin{itemize}[nosep]
\item dass sie aus der Collatz-Dynamik folgt,
\item dass sie eine Konjugation der Collatz-Abbildung liefert,
\item dass sie Aussagen über die Menge
\[
\Einf=\{n\in\Nodd:\forall K,\ U^K(n)\neq 1\}
\]
impliziert,
\item oder dass aus ihr ein Beweis der Collatz-Vermutung folgt.
\end{itemize}
Die Konstruktion dient ausschließlich dazu, die Beziehungen zwischen
EABC-Phasen, Zwölferstruktur, Bernoulli-Zahlen und den Werten der
Zetafunktion an negativen ungeraden Stellen in einem gemeinsamen
geometrischen Rahmen darzustellen.
\end{remark}
