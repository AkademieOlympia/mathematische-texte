...
\begin{tcolorbox}[colback=red!8,colframe=red!60!black,title={Korrektur (Juni 2026)}]
Eine frühere Fassung behauptete pauschal $\vii(n_{k+1}+1)\leq 2$ nach
extremalen Blöcken; das ist zu stark (Gegenbeispiel $k{=}1$, $r{=}10$).
Siehe \texttt{collatz\_angriff\_uniformitaet.tex}.
\end{tcolorbox}

\begin{tcolorbox}[ergebnis, title={Strukturelle Kompensation (präzise)}]
Proposition~\ref{prop:nachfolger} (Lean: \texttt{lte\_even\_N\_successor\_valuation\_one}):
Für gerades $N=k+r$ gilt nach dem $\mathrm{C}^k\mathrm{A}$-Block
$\vii(n_{k+1}+1)=1$; der Nachfolger ist E-Typ und startet keine C-Kette.
Für ungerades $N$ erfolgt die Kompensation im selben Block über
$\vii(3n_k+1)\geq 3$, nicht über einen pauschal flachen Nachfolger.
\end{tcolorbox}
...
$\Lambda_B \approx +0{,}54 > 0$ (LTE-schlimmster Fall)
  ...
  & Szenario B \\[2pt]
Gerades $N=k+r$: $\vii(n_{k+1}+1)=1$ nach extremalem Block
  & \textcolor{bewiesengrün}{\bfseries Bewiesen}
  & Prop.~\ref{prop:nachfolger}, Lean \\[2pt]
Pauschale $\vii(n_{k+1}+1)\leq 2$ für alle $(k,r)$
  & \textcolor{lückenrot}{\bfseries Widerlegt}
  & \texttt{collatz\_angriff\_uniformitaet.tex} \\[2pt]
...
\[
  \frac{n_{k'+1}}{n_0} < 1,
\]
wobei $k'$ die Länge der zweiten C-Kette ist; für gerades
$N=k+r$ ist $\vii(n_{k+1}+1)=1$ (Prop.~\ref{prop:nachfolger}), für
ungerades $N$ muss die Kompensation blockintern über $\vii(3n_k+1)\geq 3$
argumentiert werden.
