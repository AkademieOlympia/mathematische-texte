...
def _is_prime(n):
    """Schnelle Implementierung für Quantencomputer-Programmierung (ohne SageMath)."""
    if n < 2:
        return False
    if n == 2:
        return True
...
class EnergiedokuWaveModel:
    ...
    def load_from_quantum_computer(self, filepath=None):
        """Lädt Riemann-Nullstellen über den Makro-Quantencomputer."""
        ...
        if n > 0:
            ...
            print(f"[{self._name}] {n:,} Nullstellen vom Quantencomputer geladen.")
        return n
...
if __name__ == "__main__":
    ...
    model.load_from_quantum_computer()  # Lädt zeros6.npz/npy
    model.train_wave_model(quantum_mode=True)  # Quanten-Phasen-Interferenz
