...
# Ohne GUI: Agg + nur speichern → Skript beendet sofort.
# Fenster anzeigen: HEUREKA_SHOW=1 python Heureka.py
_SHOW = os.environ.get("HEUREKA_SHOW", "0") == "1"
if not _SHOW:
    matplotlib.use("Agg")
import matplotlib.pyplot as plt

# Riemann-Nullstellen (γ): wie in Balmer.py — sonst kleiner Fallback
...
if os.path.isfile(_ZEROS_PATH):
    ...
    try:
        ...
        _n_fallback = int(os.environ.get("HEUREKA_MP_ZEROS", "2000"))
        print(
            f"[Heureka] { _ZEROS_PATH } nicht gefunden — "
            f"berechne {_n_fallback} Nullstellen mit mpmath (kann etwas dauern).",
            file=sys.stderr,
        )
        zeros = np.array(
        ...
        print(
            "[Heureka] Weder zeros6.npy noch mpmath — nutze 40 feste γ-Werte.",
            file=sys.stderr,
        )
        zeros = np.array(
            [
...
N_test = 100160063
NUM_ZEROS = int(os.environ.get("HEUREKA_NUM_ZEROS", "500000"))
NUM_ZEROS = min(NUM_ZEROS, len(zeros))
gammas_use = zeros[:NUM_ZEROS]

# Wir "scannen" den Bereich der möglichen Faktoren (bis sqrt(N))
x_scan = np.linspace(2, int(np.sqrt(N_test)) + 100, 5000)


def spectral_probe(x, N, gammas, chunk_size=4000):
    """Summiert über γ in Blöcken — vermeidet riesige Zwischenmatrizen und Python-Overhead."""
    ...
    for start in range(0, n, chunk_size):
        ...
        if (start + chunk_size) % report_every < chunk_size or start + chunk_size >= n:
            ...
            print(f"[Heureka] Fortschritt: {pct}% ({min(start + chunk_size, n)}/{n} γ)", flush=True)
...
if __name__ == "__main__":
    print(
        f"[Heureka] N={N_test}, {len(gammas_use)} Nullstellen, {len(x_scan)} x-Punkte …",
        flush=True,
    )
    probe_results = spectral_probe(x_scan, N_test, gammas_use)

    out_png = os.path.join(os.path.dirname(__file__) or ".", "heureka_spectrum.png")
    ...
    print(f"[Heureka] Fertig. Plot: {out_png}", flush=True)
    if _SHOW:
        plt.show()
    plt.close()
