...
def summarize_top_bottom(rows: Sequence[Dict[str, object]], key: str, tail_frac: float) -> Dict[str, float]:
...
def print_table(rows: Sequence[Dict[str, object]]) -> None:
    print("=" * 128)
    print("FEINKALIBRIERUNG DES ADDITIVEN TRIGGERS")
    print("=" * 128)
    header = (
...
def print_best(rows: Sequence[Dict[str, object]]) -> None:
    ...
    print("=" * 72)
    print(
        f"Bestes Pearson  : c = {best_p['c']:.2f}, Wert = {best_p['pearson']:.6f}"
    )
    print(
        f"Bestes Spearman : c = {best_s['c']:.2f}, Wert = {best_s['spearman']:.6f}"
    )
    print(
        f"Beste Top-Bottom-Diff. : c = {best_d['c']:.2f}, Wert = {best_d['diff']*100:.2f}%"
    )


def print_examples(rows: Sequence[Dict[str, object]], c_value: float, tail_frac: float, limit: int = 12) -> None:
    augmented = []
    for r in rows:
        rr = dict(r)
        rr["T_c"] = float(rr["B_log"]) + c_value * int(rr["active_channel"])
        augmented.append(rr)

    rows_sorted = sorted(augmented, key=lambda r: float(r["T_c"]))
    n = len(rows_sorted)
    k = max(1, int(round(tail_frac * n)))
    bottom = rows_sorted[:k]
    top = rows_sorted[-k:]

    def _block(title: str, block: Sequence[Dict[str, object]]) -> None:
        print("\n" + "=" * 118)
        print(title)
        print("=" * 118)
        header = f"{'Start':>8} {'Typ':>20} {'B':>12} {'T_c':>12} {'Δ':>4} {'Σ':>4} {'ergänzbar':>10}"
        print(header)
        print("-" * len(header))
        for rr in block[:limit]:
            print(
                f"{int(rr['start']):>8} {str(rr['coarse_pattern']):>20} "
                f"{float(rr['B_log']):>12.6f} {float(rr['T_c']):>12.6f} "
                f"{int(rr['delta']):>4} {int(rr['sigma']):>4} {int(rr['extendable']):>10}"
            )

    _block(f"TOP-BEREICH für c = {c_value}", list(reversed(top)))
    _block(f"BOTTOM-BEREICH für c = {c_value}", bottom)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Feinkalibrierung des additiven Triggers T_c = B_pi + c*1_{EA-BC-BC-EA}."
    )
    ...
    parser.add_argument("--c-min", type=float, default=4.0, help="minimales c")
    parser.add_argument("--c-max", type=float, default=12.0, help="maximales c")
    parser.add_argument("--c-step", type=float, default=1.0, help="Schrittweite für c")
    parser.add_argument("--tail", type=float, default=0.10, help="Anteil für Top/Bottom-Test")
    parser.add_argument("--show-best-mode", type=str, default="pearson", choices=["pearson", "spearman", "diff", "none"])
    parser.add_argument("--show", type=int, default=12, help="Anzahl Beispielzeilen")
    ...
    for c in c_values:
        ...
        tb = summarize_top_bottom(augmented, "T_c", args.tail)

        top_sorted = sorted(augmented, key=lambda r: float(r["T_c"]))[-max(1, int(round(args.tail * len(augmented)))) :]
        top_active_frac = mean(int(r["active_channel"]) for r in top_sorted)
        ...
        result_rows.append(
            {
                ...
                "diff": tb["diff"],
                "top_active_frac": top_active_frac,
            }
        )
    ...
    print_best(result_rows)

    if args.show_best_mode != "none":
        if args.show_best_mode == "pearson":
            best = max(result_rows, key=lambda r: r["pearson"])
        elif args.show_best_mode == "spearman":
            best = max(result_rows, key=lambda r: r["spearman"])
        else:
            best = max(result_rows, key=lambda r: r["diff"])
        print_examples(rows, best["c"], args.tail, limit=args.show)
