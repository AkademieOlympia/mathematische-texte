...
from hyper_logic import apply_riemann_nav, hyper_deep_scan, hyper_resonant_shor
...
def read_grosszahl_test_choice():
    """Vorabfrage: feste Riesenzahl statt manueller Eingabe."""
    raw = input(
        "Groessentest mit fester Riesenzahl ausfuehren? (y/n): "
    ).strip().lower()
    return raw in {"y", "yes", "j", "ja"}


# Fixnum fuer den optionalen Groessentest (61-stellig)
GROSSZAHL_TEST_N = 9582734438919857565143953563990834335018402786670000053752009
...
shor = AQGShor()
if read_grosszahl_test_choice():
    target_n = GROSSZAHL_TEST_N
    print(f"[Groessentest] Verwende N mit {len(str(target_n))} Ziffern.")
else:
...
print("\n--- Hyper-NSA (Robinson-Erweiterung) ---")
hyper_shadow = hyper_resonant_shor(target_n)
print(f"Hyper-Shadow-Kandidat (sqrt-Monade): {hyper_shadow}")
if hyper_shadow > 1 and target_n % hyper_shadow == 0:
    print(f"Hyper-Shadow teilt N: {hyper_shadow} * {target_n // hyper_shadow} = {target_n}")

hyper_deep = hyper_deep_scan(target_n, hyper_shadow)
if hyper_deep is not None:
    print(f"Hyper-Deep-Scan: {hyper_deep[0]} und {hyper_deep[1]}")
else:
    riemann_nav = apply_riemann_nav(target_n, hyper_shadow)
    if riemann_nav is not None:
        print(f"Riemann-Nav Faktorpaar: {riemann_nav[0]} und {riemann_nav[1]}")
