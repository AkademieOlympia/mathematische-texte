...
class BambergQHEModel:
    ...
    # Kepler-Kopplung (aktiv): gewichtete Normierung als Hamiltonian-Skalenfaktor
    kepler_alpha: float = 1.0  # Exponent für Flächenratio
    kepler_beta: float = 1.0   # Exponent für Volumenratio
    kepler_mode: str = "multiply"  # 'multiply' oder 'divide'
    ...
    def L_magnetic(self, ridge: float = 1e-6):
        cache_key = f"L_A_r{ridge:.0e}_K{int(self.use_kepler)}_a{float(self.kepler_alpha):.3g}_b{float(self.kepler_beta):.3g}_m{str(self.kepler_mode)}"
        if cache_key in self._cache:
            return self._cache[cache_key]
        phases = self.edge_phases(ridge=ridge)
        L, m = self._assemble_sparse_L_M(phases=phases)
        # Aktive Kepler-Kopplung: skaliere den Operator L (Topologie/Chern bleibt im Flux-Teil).
        if bool(self.use_kepler):
            sK = float(self.kepler_scale_factor())
            if str(self.kepler_mode).lower() == "divide":
                L = (L * (1.0 / max(sK, 1e-18))).tocsr()
            else:
                L = (L * sK).tocsr()
    ...
    # --------------------
    # Landau-Level Analyse (physikalische Zuordnung)
    # --------------------
    def landau_analysis(self, ridge: float = 1e-6, k: int = 40, mass_kg: Optional[float] = None) -> Dict[str, Any]:
        """
        Landau-Analyse für Elektron auf (annähernd) kugelförmiger Fläche.

        Interpretation:
          - Gesamtfluss in Einheiten von Φ0=h/e:  Nphi = flux_p / flux_q
          - effektives B-Feld:  B = Nphi * Φ0 / A_sphere   mit A_sphere=4πR^2
          - Zyklotronfrequenz: ωc = e B / m
          - Landau-Niveaus (planar): E_n = ħ ωc (n + 1/2)

        Vergleich:
          Wir mappen unsere diskreten Eigenwerte λ über E = ħ² λ/(2m) in Joule/eV
          und vergleichen grob die Abstände zu ħωc.
        """
        if mass_kg is None:
            mass_kg = float(self.si.m_e)
        m = float(mass_kg)

        # Nphi as rational
        p, q = int(self.flux[0]), int(self.flux[1])
        Nphi = float(p) / float(q) if q != 0 else float(p)

        R = float(self.radius_m)
        A = 4.0 * math.pi * (R**2)

        Phi0 = float(self.si.h / self.si.e)  # Φ0 = h/e
        B = (Nphi * Phi0) / max(A, 1e-300)

        e = float(self.si.e)
        hbar = float(self.si.hbar)

        omega_c = e * B / m
        dE_L = hbar * omega_c  # Landau spacing (J)

        # model spectrum energies
        lev = self.energy_levels_si(ridge=ridge, k=int(k), mass_kg=m)
        E = np.array(lev["E_J"], dtype=float)
        # use positive sorted energies (should already be)
        E = np.sort(E)

        # estimate cluster spacing by differences
        dE = np.diff(E[:min(len(E), 20)])
        # robust: median of small diffs >0
        dEpos = dE[dE > 0]
        dE_med = float(np.median(dEpos)) if dEpos.size else float("nan")

        # theoretical Landau energies for n=0..min(10,k)
        nmax = min(10, int(k)-1)
        En = [float(dE_L * (n + 0.5)) for n in range(nmax+1)]
        En_eV = [x / 1.602176634e-19 for x in En]
        ...
        return {
            ...
            "landau_spacing_J": float(dE_L),
            "landau_spacing_eV": float(dE_L / 1.602176634e-19),
            "model_spacing_median_J": float(dE_med),
            "model_spacing_median_over_landau": (float(dE_med / dE_L) if (dE_L>0 and math.isfinite(dE_med)) else None),
            "En_J": En,
            "En_eV": En_eV,
            "note": "Planare Landau-Formeln; auf der Kugel (Haldane) gibt es Krümmung/Monopol-Korrekturen."
        }
    ...
    def kepler_invariants(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "kepler_volume_root": float(self.kepler_volume_root), "kepler_alpha": float(self.kepler_alpha), "kepler_beta": float(self.kepler_beta), "kepler_mode": str(self.kepler_mode),
            "A_poly": float(A),
            "V_poly": float(V),
    ...
    def kepler_scale_factor(self) -> float:
        """
        Aktiver Kepler-Kopplungsfaktor (dimensionslos) für den Hamilton-Operator.

        Definition (standard):
          rA = A_poly / A_sphere
          rV = V_poly / V_sphere
          s  = (rA**kepler_alpha) * (rV**kepler_beta)

        Anwendung:
          - kepler_mode='multiply': L_eff = s * L
          - kepler_mode='divide'  : L_eff = L / s

        Interpretation:
          s < 1 bedeutet: Polyeder "unterfüllt" gegenüber Kugel (bei R fix).
          Mit s als Kopplung kannst du Energieniveaus / Zeitskalen systematisch verschieben,
          ohne Geometrie/Topologie (Chern) zu zerstören.
        """
        inv = self.kepler_invariants()
        rA = float(inv["A_poly_over_A_sphere"])
        rV = float(inv["V_poly_over_V_sphere"])
        # safeguard
        rA = max(rA, 1e-18)
        rV = max(rV, 1e-18)
        s = (rA ** float(self.kepler_alpha)) * (rV ** float(self.kepler_beta))
        return float(s)
    ...
    # --------------------
    # SI mapping: eigenvalues -> energies/frequencies via electron mass
    # --------------------
    def energy_levels_si(self, ridge: float = 1e-6, k: int = 30, mass_kg: Optional[float] = None) -> Dict[str, Any]:
        """
        Mappe Eigenwerte λ (≈ l(l+1)/R^2) auf kinetische Energien eines Elektrons auf der Kugel:
            E = (ħ^2 / (2 m)) * λ
        (für skalaren Laplace-Operator / Schrödinger-Kinetik).

        Rückgabe in Joule + optional eV, plus ω und f.
        """
        if mass_kg is None:
            mass_kg = float(self.si.m_e)
        self.eig_k = int(k)
        sp = self.spectrum_magnetic(ridge=ridge)
        lam = np.array(sp["eigs"], dtype=float)[:k]

        hbar = float(self.si.hbar)
        m = float(mass_kg)

        E = (hbar**2) * lam / (2.0*m)               # Joule
        omega = E / hbar                             # rad/s
        f = omega / (2.0*np.pi)                      # Hz
        eV = E / 1.602176634e-19                     # eV
        fC = float(self.si.compton_frequency)
        f_over_fC = f / max(fC, 1e-300)

        return {
            "ridge": float(ridge),
            "k": int(k),
            "mass_kg": float(m),
            "eigs_lambda": [float(x) for x in lam],
            "E_J": [float(x) for x in E],
            "E_eV": [float(x) for x in eV],
            "omega_rad_s": [float(x) for x in omega],
            "f_Hz": [float(x) for x in f],
            "f_over_fCompton": [float(x) for x in f_over_fC],
            "fCompton_Hz": float(fC),
            "note": "E = ħ² λ / (2m). Kepler-Skalierung wirkt über λ auf E,f."
        }


    def qhe_plateau(self, nu: float) -> Dict[str, Any]:
        """
        Idealisiertes QHE-Plateau (Integer/Fractional):
          R_xy = R_K / ν
          σ_xy = ν * (e^2/h)  (Leitwert)
        """
        nu = float(nu)
        RK = float(self.si.von_klitzing)
        G0 = float(self.si.conductance_quantum)
        if abs(nu) < 1e-18:
            return {"error": "nu darf nicht 0 sein."}
        Rxy = RK / nu
        sigma_xy = nu * G0
        return {
            "nu": nu,
            ...
            "G0_S": G0,
            "Rxy_ohm": float(Rxy),
            "sigma_xy_S": float(sigma_xy),
            "note": "Ideal-Plateau ohne Dissipation/Disorder."
        }

    # --------------------
    # Projektor-Chern mit Stereo-Charts (Nord/Süd) + Blend
    # --------------------
    def chern_projector_marker_stereo(self, ridge: float = 1e-6, n_occ: Optional[int] = None,
                                      eps: float = 1e-6, blend_kappa: float = 4.0) -> Dict[str, Any]:
        """
        Projektor-Chern-Diagnose auf der Kugel via zwei stereographische Charts (Nord/Süd)
        mit *weicher* Partition-of-Unity (POU)-Blend-Gewichtung.

        Wir berechnen je Chart:
            C_chart = -2π Im Tr( W_chart P [X,P][Y,P] )
        mit W_chart = diag(w_i) als Gewichtsmatrix (POU).
        """
        spc = self.spectrum_magnetic(ridge=ridge)
        eigs = spc["eigs"]
        vecs = spc["vecs"]
        if vecs is None:
            return {"error": "Eigenvektoren fehlen. Setze store_eigenvectors=True und berechne spectrum_magnetic."}
        ...
        # physical coords
        Xv = self.vertices.astype(float) * float(self.radius_m)
        x3, y3, z3 = Xv[:,0], Xv[:,1], Xv[:,2]
        R = float(self.radius_m)

        # weights (smooth): wN ~ 1 for z>0; wS ~ 1 for z<0
        # use tanh for smooth transition (kappa controls sharpness)
        wN = 0.5*(1.0 + np.tanh(float(blend_kappa) * (z3 / max(R, 1e-18))))
        wS = 1.0 - wN

        # North stereographic from (0,0,R): onto z=-R
        denomN = (R - z3)
        denomN = np.where(np.abs(denomN) < eps, np.sign(denomN)*eps + eps, denomN)
        xN = (2*R) * x3 / denomN
        yN = (2*R) * y3 / denomN

        # South stereographic from (0,0,-R): onto z=+R
        denomS = (R + z3)
        denomS = np.where(np.abs(denomS) < eps, np.sign(denomS)*eps + eps, denomS)
        xS = (2*R) * x3 / denomS
        yS = (2*R) * y3 / denomS

        def chern_weighted(xs, ys, w):
            ...
            term = P @ XP @ YP
            # weighted trace
            C = -2.0*np.pi * np.imag(np.sum(np.diag(term) * w))
            return float(C)

        CN = chern_weighted(xN, yN, wN)
        CS = chern_weighted(xS, yS, wS)
        Cblend = CN + CS
        ...
        return {
            ...
            "north": {"C": float(CN)},
            "south": {"C": float(CS)},
            "blend": {"C": float(Cblend)},
            "weights": {"wN_mean": float(np.mean(wN)), "wS_mean": float(np.mean(wS)), "blend_kappa": float(blend_kappa)},
            "note": "POU-Blend: Cblend = CN(wN) + CS(wS). Diagnose; auf feinen Meshes stabiler."
        }

    # --------------------
    # Lz-Operator (diskret, nachbarschaftsbasiert) + Diagnostik
    # --------------------
    def _neighbor_list(self) -> List[List[int]]:
        """Adjazenzliste aus den Faces."""
        n = self.nV
        nbr = [set() for _ in range(n)]
        for (i,j,k) in self.faces.astype(int):
            nbr[i].update([j,k]); nbr[j].update([i,k]); nbr[k].update([i,j])
        return [sorted(list(s)) for s in nbr]

    def _grad_operators_xy(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Baue lokale Least-Squares Gradient-Operatoren Dx, Dy (dense) auf dem Vertex-Graphen.
        Für kleine Meshes ok; für große später sparse/iterativ.
        """
        n = self.nV
        nbr = self._neighbor_list()
        V = (self.vertices.astype(float) * float(self.radius_m))
        ...
        for i in range(n):
            ...
            for j in neigh:
                xj, yj, zj = V[j]
                A.append([xj-xi, yj-yi])  # local planar approximation in xy
            A = np.array(A, dtype=float)
            # least squares pseudo-inverse
            # gradient g satisfies A @ g ≈ f_j - f_i
            pinv = np.linalg.pinv(A)
            # each neighbor contributes weight row in pinv
            for row, j in enumerate(neigh):
                w = pinv[:, row]
                Dx[i, j] += w[0]
                Dy[i, j] += w[1]
            Dx[i, i] -= Dx[i, neigh].sum()
            Dy[i, i] -= Dy[i, neigh].sum()

        return Dx, Dy


    def _grad_operators_xy_sparse(self) -> Tuple["sp.csr_matrix", "sp.csr_matrix"]:
        """
        Sparse Variante der lokalen LS-Gradienten-Operatoren Dx, Dy.
        Für größere Meshes deutlich schneller/speichersparender als dense.
        """
        ...
        rows_dx, cols_dx, data_dx = [], [], []
        rows_dy, cols_dy, data_dy = [], [], []
        ...
        for i in range(n):
            ...
            if len(neigh) < 2:
                ...
                continue
            xi, yi, zi = V[i]
            A = np.array([[V[j,0]-xi, V[j,1]-yi] for j in neigh], dtype=float)  # len(neigh) x 2
            pinv = np.linalg.pinv(A)  # 2 x len(neigh)

            # neighbor weights
            dx_row_sum = 0.0
            dy_row_sum = 0.0
            for col_idx, j in enumerate(neigh):
                w0 = float(pinv[0, col_idx])
                w1 = float(pinv[1, col_idx])

                if abs(w0) > 0:
                    rows_dx.append(i); cols_dx.append(j); data_dx.append(w0); dx_row_sum += w0
                if abs(w1) > 0:
                    rows_dy.append(i); cols_dy.append(j); data_dy.append(w1); dy_row_sum += w1

            # diagonal to enforce (row sum)=0
            if abs(dx_row_sum) > 0:
                rows_dx.append(i); cols_dx.append(i); data_dx.append(-dx_row_sum)
            if abs(dy_row_sum) > 0:
                rows_dy.append(i); cols_dy.append(i); data_dy.append(-dy_row_sum)

        Dx = sp.csr_matrix((data_dx, (rows_dx, cols_dx)), shape=(n, n))
        Dy = sp.csr_matrix((data_dy, (rows_dy, cols_dy)), shape=(n, n))
        return Dx, Dy

    def Lz_operator(self, hbar: Optional[float] = None, sparse: bool = True):
        """
        Diskreter Generator der z-Rotation:
            Lz = - i ħ ( x d/dy - y d/dx )
        basierend auf lokalen LS-Gradienten in der xy-Ebene.

        sparse=True: liefert CSR-Matrix (empfohlen).
        """
        if hbar is None:
            hbar = float(self.si.hbar)
        ...
        if sparse:
            import scipy.sparse as sp
            Dx, Dy = self._grad_operators_xy_sparse()
            V = (self.vertices.astype(float) * float(self.radius_m))
            x = V[:, 0].astype(float)
            y = V[:, 1].astype(float)
            X = sp.diags(x, 0, format="csr")
            Y = sp.diags(y, 0, format="csr")
            Lz = (-1j * float(hbar)) * (X @ Dy - Y @ Dx)
            return Lz.tocsr()
        else:
            Dx, Dy = self._grad_operators_xy()
            V = (self.vertices.astype(float) * float(self.radius_m))
            x = V[:,0]; y = V[:,1]
            Lz = (-1j*float(hbar)) * (np.diag(x) @ Dy - np.diag(y) @ Dx)
            return Lz

    def Lz_diagnostics(self, ridge: float = 1e-6, k: int = 12, n_occ: Optional[int] = None) -> Dict[str, Any]:
        """
        Diagnostik: Erwartungswerte <ψ|Lz|ψ> und <ψ|Lz^2|ψ> für die ersten Eigenzustände.
        Zusätzlich: Vergleich zur L^2-Schätzung aus λ (für Kugel quantisiert).
        """
        self.eig_k = int(max(k, 20))
        self.store_eigenvectors = True
        sp = self.spectrum_magnetic(ridge=ridge)
        vecs = sp["vecs"]
        lam = np.array(sp["eigs"], dtype=float)
        if vecs is None:
            return {"error": "Eigenvektoren fehlen."}

        Lz = self.Lz_operator(sparse=True)
        Lz2 = Lz @ Lz
        ...
        rows = []
        for n in range(min(k, vecs.shape[1])):
            psi = np.array(vecs[:, n], dtype=complex)
            psi = psi / np.linalg.norm(psi)
            exp_Lz = float(np.real(np.vdot(psi, Lz @ psi)))
            exp_Lz2 = float(np.real(np.vdot(psi, Lz2 @ psi)))
            # dimensionless m_est in units of ħ
            m_est = exp_Lz / hbar
            # l-est from lambda
            x = float(lam[n]) * (R**2)
            l_est = 0.5 * (-1.0 + math.sqrt(max(0.0, 1.0 + 4.0*x)))
            rows.append({
                "state": int(n),
                "lambda": float(lam[n]),
                "l_est": float(l_est),
                "Lz_Js": exp_Lz,
                "Lz_over_hbar": float(m_est),
                "Lz2_J2s2": exp_Lz2,
                "sqrt_Lz2_over_hbar": float(math.sqrt(max(exp_Lz2,0.0))/hbar),
            })
        ...
        return {
            ...
            "k": int(k),
            "rows": rows,
            "note": "Auf groben Meshes ist Lz nur eine qualitative Diagnose; auf feineren Levels wird m-Quantisierung klarer."
        }


    def Lz_shell_splitting(self, ridge: float = 1e-6, k: int = 40, tol: float = 1e-6) -> Dict[str, Any]:
        """
        Quantisierungstest: in (nahezu) entarteten Eigenraum-Blöcken (Laplacian-Shells)
        diagonalisiere Lz, um m-Werte zu extrahieren.

        Vorgehen:
          1) berechne k Eigenpaare (λ_n, ψ_n)
          2) clustere λ nach |λ_i-λ_j| <= tol
          3) in jedem Cluster: M_ab = <ψ_a|Lz_H|ψ_b>, Lz_H = (Lz+Lz†)/2
          4) Eigenwerte von M_ab / ħ ~ m
        """
        self.eig_k = int(max(k, 20))
        self.store_eigenvectors = True
        sp = self.spectrum_magnetic(ridge=ridge)
        lam = np.array(sp["eigs"], dtype=float)[:k]
        vecs = np.array(sp["vecs"][:, :k], dtype=complex)
        if vecs is None:
            return {"error": "Eigenvektoren fehlen."}

        hbar = float(self.si.hbar)
        Lz = self.Lz_operator(hbar=hbar, sparse=True)
        # hermitize for stability
        LzH = 0.5*(Lz + Lz.getH())
        ...
        for i in range(k):
            ...
            for j in range(i+1, k):
                if not used[j] and abs(lam[j]-lam[i]) <= tol:
                    used[j] = True
                    idx.append(j)
            clusters.append(idx)
        ...
        for idx in clusters:
            r = len(idx)
            if r == 1:
                rows.append({"lambda_center": float(lam[idx[0]]), "size": 1, "m_over_hbar": [0.0]})
                continue
            Psi = vecs[:, idx]  # nV x r
            # Orthonormalize (QR)
            ...
            mvals = (np.real(w) / hbar).tolist()
            rows.append({
                "lambda_center": float(np.mean(lam[idx])),
                "size": int(r),
                "m_over_hbar": [float(x) for x in mvals],
                "m_rounded": [int(round(x)) for x in mvals]
            })
        ...
        return {
            ...
            "tol": float(tol),
            "rows": rows,
            "note": "Für reine Laplace-Shells (ohne Flux) sollten m-Integer im Entartungsraum sichtbar werden."
        }
    ...
    def to_dict_meta(self) -> Dict[str, Any]:
        return {
            ...
            "geometry": {"nV": self.nV, "nF": self.nF, "radius_m": float(self.radius_m), "use_kepler": bool(self.use_kepler), "kepler_area_root": float(self.kepler_area_root), "kepler_volume_root": float(self.kepler_volume_root), "kepler_alpha": float(self.kepler_alpha), "kepler_beta": float(self.kepler_beta), "kepler_mode": str(self.kepler_mode)},
            "params": {
    ...
    def load(cls, folder: str | Path) -> "BambergQHEModel":
        ...
        obj = cls(
            ...
            kepler_alpha=float(meta["geometry"].get("kepler_alpha", 1.0)),
            kepler_beta=float(meta["geometry"].get("kepler_beta", 1.0)),
            kepler_mode=str(meta["geometry"].get("kepler_mode", "multiply")),
            si=si,
...
def main():
    ...
    ap_kset = sub.add_parser("kepler-set")

    ap_levels = sub.add_parser("levels-si")
    
    ap_qhe = sub.add_parser("qhe")
    
    ap_landau = sub.add_parser("landau")
    ap_landau.add_argument("--state", required=True)
    ap_landau.add_argument("--ridge", type=float, default=1e-6)
    ap_landau.add_argument("--k", type=int, default=40)

    ap_qhe.add_argument("--state", required=True)
    ap_qhe.add_argument("--nu", type=float, default=1.0)

    ap_levels.add_argument("--state", required=True)
    ap_levels.add_argument("--ridge", type=float, default=1e-6)
    ap_levels.add_argument("--k", type=int, default=20)

    ap_pst = sub.add_parser("projector-chern-stereo")
    ap_pst.add_argument("--state", required=True)
    ap_pst.add_argument("--ridge", type=float, default=1e-6)
    ap_pst.add_argument("--n-occ", type=int, default=0)

    ap_lz = sub.add_parser("lzdiag")
    
    ap_lzs = sub.add_parser("lzshell")
    ap_lzs.add_argument("--state", required=True)
    ap_lzs.add_argument("--ridge", type=float, default=1e-6)
    ap_lzs.add_argument("--k", type=int, default=40)
    ap_lzs.add_argument("--tol", type=float, default=1e-6)

    ap_lz = sub.add_parser("lzdiag")
    ap_lz.add_argument("--state", required=True)
    ap_lz.add_argument("--ridge", type=float, default=1e-6)
    ap_lz.add_argument("--k", type=int, default=12)
    ap_kset.add_argument("--state", required=True)
    ap_kset.add_argument("--use", type=int, default=1)
    ap_kset.add_argument("--alpha", type=float, default=1.0)
    ap_kset.add_argument("--beta", type=float, default=1.0)
    ap_kset.add_argument("--mode", type=str, default="multiply")
    ...
    elif args.cmd == "kepler-set":
        m = BambergQHEModel.load(args.state)
        m.use_kepler = bool(int(args.use))
        m.kepler_alpha = float(args.alpha)
        m.kepler_beta = float(args.beta)
        m.kepler_mode = str(args.mode)
        m.save(args.state)
        out = {"saved": True, "use_kepler": m.use_kepler, "kepler_alpha": m.kepler_alpha, "kepler_beta": m.kepler_beta, "kepler_mode": m.kepler_mode, "kepler_scale_factor": m.kepler_scale_factor()}
        print("[kepler-set]", json.dumps(out, indent=2))

    elif args.cmd == "levels-si":
        m = BambergQHEModel.load(args.state)
        out = m.energy_levels_si(ridge=float(args.ridge), k=int(args.k))
        print("[levels-si]", json.dumps(out, indent=2))

    elif args.cmd == "projector-chern-stereo":
        m = BambergQHEModel.load(args.state)
        m.store_eigenvectors = True
        n_occ = int(args.n_occ) if int(args.n_occ) > 0 else None
        out = m.chern_projector_marker_stereo(ridge=float(args.ridge), n_occ=n_occ)
        print("[projector-chern-stereo]", json.dumps(out, indent=2))

    elif args.cmd == "lzdiag":
        m = BambergQHEModel.load(args.state)
        out = m.Lz_diagnostics(ridge=float(args.ridge), k=int(args.k))
        print("[lzdiag]", json.dumps(out, indent=2))

    elif args.cmd == "lzshell":
        m = BambergQHEModel.load(args.state)
        out = m.Lz_shell_splitting(ridge=float(args.ridge), k=int(args.k), tol=float(args.tol))
        print("[lzshell]", json.dumps(out, indent=2))

    elif args.cmd == "qhe":
        m = BambergQHEModel.load(args.state)
        out = m.qhe_plateau(nu=float(args.nu))
        print("[qhe]", json.dumps(out, indent=2))

    elif args.cmd == "landau":
        m = BambergQHEModel.load(args.state)
        out = m.landau_analysis(ridge=float(args.ridge), k=int(args.k))
        print("[landau]", json.dumps(out, indent=2))
