...
def primquaternionen():
    ...
    return {_sage_const_5 : pi5, _sage_const_7 : pi7}
...
def schwung_resonanz_modulo(u, primes=(_sage_const_5 , _sage_const_7 )):
...
def schwung_report(q, q_prime, label_q="q", label_qp="q'", *, verbose=True):
    ...
    any_norm = any(res[p]["N_resonanz"] for p in (_sage_const_5 , _sage_const_7 ))
    any_ideal = any(res[p]["I_p"] for p in (_sage_const_5 , _sage_const_7 ))
    return {
        "u": eabc_from_q(u),
        "N_u": nu,
        "N_q": nq,
...
def scan_schwung_resonanz(M, *, data_dir=None, events_csv=None, trans_csv=None):
    ...
    if transitions:
        ...
        for row in transitions:
            ...
            if comps == (_sage_const_0 , _sage_const_0 , _sage_const_0 , _sage_const_0 ) or comps in seen:
                continue
            seen.add(comps)
    ...
    if events:
        ...
        for k in range(len(labels) - _sage_const_1 ):
            qk = quaternion_from_label(labels[k])
            qkp1 = quaternion_from_label(labels[k + _sage_const_1 ])
            ...
            label_pairs.append({
                "k": k,
                "labels": (labels[k], labels[k + _sage_const_1 ]),
                "u": eabc_from_q(u),
                "N": q_norm(u),
                "res": res,
            })
    ...
    n_norm5 = sum(_sage_const_1  for p in pairs if p["res"][_sage_const_5 ]["N_resonanz"])
    n_norm7 = sum(_sage_const_1  for p in pairs if p["res"][_sage_const_7 ]["N_resonanz"])
    n_i5 = sum(_sage_const_1  for p in pairs if p["res"][_sage_const_5 ]["I_p"])
    n_i7 = sum(_sage_const_1  for p in pairs if p["res"][_sage_const_7 ]["I_p"])
    ...
    for p in pairs[:_sage_const_12 ]:
        r5, r7 = p["res"][_sage_const_5 ], p["res"][_sage_const_7 ]
        print(
            f"  u={p['u']}  N={p['N']}  "
    ...
    if len(pairs) > _sage_const_12 :
        print(f"  ... ({len(pairs) - _sage_const_12 } weitere)")

    ln_norm5 = sum(_sage_const_1  for p in label_pairs if p["res"][_sage_const_5 ]["N_resonanz"])
    ln_norm7 = sum(_sage_const_1  for p in label_pairs if p["res"][_sage_const_7 ]["N_resonanz"])
    ln_i5 = sum(_sage_const_1  for p in label_pairs if p["res"][_sage_const_5 ]["I_p"])
    ln_i7 = sum(_sage_const_1  for p in label_pairs if p["res"][_sage_const_7 ]["I_p"])
...
def test_zustaende():
    ...
    q_glatt = q_from_eabc(*kollaps_eabc(_sage_const_5 , _sage_const_2 , _sage_const_0 , _sage_const_1 ))
    q_e = q_from_eabc(_sage_const_1 , _sage_const_0 , _sage_const_0 , _sage_const_0 )

    print("Referenzzustände:")
    print(f"  q_lokal = {eabc_from_q(q_lokal)},  Π_Γ → {kollaps_eabc(_sage_const_5 , _sage_const_2 , _sage_const_0 , _sage_const_1 )}")
...
def main():
    ...
    scan = scan_schwung_resonanz(_sage_const_113160 )
