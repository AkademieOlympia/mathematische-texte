...
% ====================================================
\section{Verbindung zu \texorpdfstring{$\zeta(2n)$}{zeta(2n)} und Riemann-Sphäre}
\label{sec:riemann}
% ====================================================

\subsection{Die Formel von Euler}

Für $\zeta(2n) = \sum_{k=1}^{\infty} k^{-2n}$ gilt die Formel von Euler (1740):
\[
...
Proposition~\ref{prop:zeta-verbindung} liefert eine direkte Verbindung:
\[
  \boxed{\In(n) \;=\; \frac{2\,(2n)!}{\,(2\pi)^{2n}} \cdot \zeta(2n) \cdot \Denom{n}.}
\]
Das bedeutet: Die Bernoulli-Normschale $\In(n)$ ist proportional zum
Riemann-Zeta-Wert $\zeta(2n)$, skaliert mit $(2n)!/(2\pi)^{2n}$ (was für
großes $n$ super-exponentiell anwächst) und dem Faktor $\Denom{n}$.

Entlang einer Collatz-Trajektorie $(n_0, n_1, n_2, \ldots)$ nimmt $\In(n_k)$
also Werte $\zeta(2n_k)$ (skaliert und ganzzahlig gemacht) an.
\end{tcolorbox}

\subsection{Strukturelle Bemerkungen zur Riemann-Hypothese}

Da $\zeta(2n) \to 1$ für $n \to \infty$ (geometrisch schnell: $\zeta(2n) - 1 \sim 2^{-2n}$),
dominiert der Faktor $(2n)!/(2\pi)^{2n}$ das Wachstum vollständig.
Die Zeta-Werte $\zeta(2n)$ tragen nur eine bescheidene
$O(1)$-Korrektur zum logarithmischen Wachstum bei:
\[
  \log\In(n) = \log\frac{2(2n)!}{(2\pi)^{2n}} + \log\zeta(2n) + \log\Denom{n}.
\]
Der zweite Term $\log\zeta(2n) \to 0$ ist vernachlässigbar; der Hauptterm ist
$\log(2n)! - 2n\log(2\pi) \sim 2n\log(2n/e)$ nach Stirling.

\begin{bemerkung}[Zur Riemann-Sphäre]
Der Ausdruck $\In(n) \propto \zeta(2n) \cdot (2n)!/\pi^{2n}$ hat eine analytische
Fortsetzung: $\zeta(s)$ ist meromorph auf $\C$ mit einfachem Pol bei $s = 1$.
Die Einschränkung auf $s = 2n$ (positive gerade ganze Zahlen) wählt spezielle
Werte auf der reellen Achse der Riemann-Sphäre aus.
Die Collatz-Trajektorie ``tastet'' damit die Werte $\zeta(2n_k)$ für die Folge
$(n_k)$ der (halbierten) Collatz-Iterate ab --- ein strukturell
interessanter Gesichtspunkt, auch wenn er keinen direkten Beweiswert hat.
\end{bemerkung}

\begin{bemerkung}[Verbindung zur Riemann-Hypothese]
Die Riemann-Hypothese betrifft die Nullstellen von $\zeta(s)$ in der
kritischen Streifen $0 < \text{Re}(s) < 1$; auf der reellen Achse bei $s = 2n > 0$
hat $\zeta$ keine Nullstellen. Die Verwendung von $\zeta(2n)$ in $\In(n)$
ist daher \emph{nicht} direkt mit der Riemann-Hypothese verknüpft.
\end{bemerkung}

\subsection{Asymptotik via Stirling und Zeta}

Kombiniert man alle Terme:
\begin{align}
...
und der Darstellung $\In(n) \propto \zeta(2n)\cdot(2n)!/(2\pi)^{2n}$.
