...
// ring_bench_e8_ecm_v6.cpp
// Benchmark harness for 128/256-bit integers:
// - Miller–Rabin timing (primality test)
// - E8 factor-finder timing (hook; replace with your real algorithm)
// - ECM (Lenstra) stage-1 factorization (replaces Pollard–Rho)
//
// Build (macOS/Linux, Boost is header-only for multiprecision):
//   g++ -std=c++17 -O3 -march=native -o ring_bench_e8_ecm_v6 ring_bench_e8_ecm_v6.cpp
//
// Quick start:
//   ./ring_bench_e8_ecm_v6 --bits 128 --count 10 --seed 42 --save-dataset dataset.csv --e8 off --out res_no_e8.csv
//   ./ring_bench_e8_ecm_v6 --load-dataset dataset.csv --e8 on --e8-ms 50 --ecm-ms 3000 --out res_e8.csv
//
// Output:
...
static bool get_local_energy_state(const Int& n, int mr_rounds, std::mt19937_64& rng,
    ...
    if (n < 4) return false;
    Int root = integer_sqrt(n);
    if (root < 2) root = 2;
    Int Q[4];
    const int residues[4] = { 1, 5, 7, 11 };
    for (int i = 0; i < 4; ++i) {
        Q[i] = find_anchor_prime(root, residues[i], mr_rounds, rng);
        if (Q[i] == 0) return false;
    }
    calculate_metrics<Int>(Q, s_sk, s_i, r_q);
    return true;
}

// Sensitivity und Scale-Multiplikator aus energetischem Zustand
template<class Int>
static void get_energy_tuned_params(const Int& n, int mr_rounds, std::mt19937_64& rng,
    ...
    double s_sk, s_i, r_q;
    if (!get_local_energy_state(n, mr_rounds, rng, s_sk, s_i, r_q)) return;
...
// ---------------- E8 hook (mit energetischem Vor-Verstärker) ----------------
...
static Args parse(int argc, char** argv) {
    ...
    for (int i=1;i<argc;++i) {
        ...
        if (k=="--bits") a.bits=(unsigned)std::stoul(need(k));
            ...
            std::cout <<
              "Usage: ./ring_bench_e8_ecm_v6 [options]\n\n"
              "Dataset:\n"
