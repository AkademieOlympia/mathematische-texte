...
\newcommand{\kk}{\bm{k}}
\newcommand{\vx}{\bm{x}} % \x umbenannt, vermeidet Konflikte
\newcommand{\cD}{\mathcal{D}} % \D umbenannt, LaTeX-Akzent
...
\begin{document}
	...
	A recent formalization in Lean has shown that a widely cited 2006 criterion is incorrect: a condition stated in terms of the directional functions \(J_2\) and \(J_4\) is necessary but not sufficient for global stability. In particular, a purely directional positivity condition was incorrectly taken to imply boundedness from below of the full potential.
	...
	\begin{equation}\label{eq:reduced-potential}
		V(K_0,\kk)=K_0J_2(\kk)+K_0^2J_4(\kk),
	\end{equation}
	where \(J_2\) is affine and \(J_4\) is quadratic in \(\kk\). This reduction is standard in the literature and is also the form used in the recent formal analysis.
	...
	\[
	\cD(\kk):=J_2(\kk),\qquad \cQ(\kk):=J_4(\kk).
	\]
	We interpret \(\cD\) as a first-order drift functional and \(\cQ\) as a second-order curvature functional.
	
	\begin{definition}[Coupled coercivity]
		We say that the pair \((\cD,\cQ)\) is \emph{uniformly coercive on \(\Sphere\)} if there exists \(c\ge 0\) such that
		\begin{equation}\label{eq:uniform-coercivity}
			\cQ(\kk)\ge 0
			\quad\text{and}\quad
			\cD(\kk)^2\le 4c\,\cQ(\kk)
			\quad\text{whenever }\cD(\kk)<0,
	...
	\begin{corollary}\label{cor:coercive-formulation}
		Global stability of the reduced 2HDM potential is equivalent to uniform coercivity of the pair \((\cD,\cQ)\) on the admissible direction space \(\Sphere\).
	\end{corollary}
	
	This formulation makes the geometric content transparent:
	\begin{itemize}
		\item \(\cD\) determines the preferred downward drift along a direction;
		\item \(\cQ\) determines the restoring curvature along that direction;
	...
	\begin{equation}\label{eq:effective-radial}
		\mathscr{E}(\kk):=\inf_{K_0>0}\bigl(K_0\cD(\kk)+K_0^2\cQ(\kk)\bigr).
	\end{equation}
	...
	\begin{proposition}\label{prop:effective-energy}
		...
		\mathscr{E}(\kk)=
		\begin{cases}
			0, & \text{if }\cD(\kk)\ge 0 \text{ and }\cQ(\kk)\ge 0,\\[0.5em]
			-\dfrac{\cD(\kk)^2}{4\cQ(\kk)}, & \text{if }\cD(\kk)<0 \text{ and }\cQ(\kk)>0,\\
			-\infty, & \text{if }\cQ(\kk)<0 \text{ or if } \cQ(\kk)=0 \text{ and }\cD(\kk)<0.
		\end{cases}
		\]
	\end{proposition}
	
	\begin{proof}
		This follows by minimizing explicitly in \(K_0\) using \cref{lem:quadratic-completion}, together with the degenerate cases \(\cQ(\kk)=0\) and \(\cQ(\kk)<0\).
	\end{proof}
	...
	Thus the stability problem can be rephrased as boundedness from below of an induced effective energy on direction space. This is often the most economical way to see why purely directional sign conditions are insufficient: one must control the \emph{ratio} \(\cD^2/\cQ\), not merely the signs of \(\cD\) and \(\cQ\).
	...
	\begin{equation}\label{eq:direction-matrix}
		M(\kk):=
		\begin{pmatrix}
			0 & \frac12 \cD(\kk)\\[0.3em]
			\frac12 \cD(\kk) & \cQ(\kk)
		\end{pmatrix}.
	\end{equation}
	Then
	\begin{equation}\label{eq:bilinear-form}
		u(K_0)^{\!T} M(\kk)\,u(K_0)=K_0\cD(\kk)+K_0^2\cQ(\kk)=V(K_0,\kk).
	\end{equation}
	...
	Suppose one replaces the scalar pair \((\cD,\cQ)\) by operators \((D_1,D_2)\) on a Hilbert space \(\mathcal{H}\), and considers a scale-dependent family
	\[
	...
	This is the real content hidden behind the formal correction. The recent Lean formalization did not merely identify a false theorem; it exposed the need for a more precise analytic language for the stability problem itself.
	...
	\begin{axiom}[Kanonische Schwellen]
		...
		Die Größen \(\pi_{\text{bal}}^M(n)\) und \(\pi_{\text{wech}}^M(n)\) werden als die kanonischen Schwellen des Modells betrachtet. Sie kodieren den ersten Balancepunkt bzw. die erste Dominanzumkehr zwischen Schalenmasse und Restmasse.
	\end{axiom}
	...
	\begin{bemerkung}
		...
		\[
		\pi_{\text{bal}}^R(n)=\pi_{\text{wech}}^R(n)=q.
		\]
	...
	\begin{definition}[Arbeitsprogramm]
		...
		\begin{enumerate}
			\item Bestimme für gegebene Zahlen \(n\in\mathcal M\) die Größen \(S_k(n)\), \(R_k(n)\), \(L_k^M(n)\), \(\pi_{\text{bal}}^M(n)\), \(\pi_{\text{wech}}^M(n)\).
