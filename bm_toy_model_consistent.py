...
# ------------------------------------------------------------
# Konsistente Toy-Version mit neuer theta*-Projektionsformel
# ------------------------------------------------------------
...
def toy_local_bias(N: int, offset: int, scale: float = 0.035) -> np.ndarray:
    """
    Kleiner dimensionsloser mod-4-Bias-Proxy.
    Explizit Toy-Kalibrierung, bis echter Primfenster-Bias eingebaut ist.
    """
...
def theta_star_proxy_new(T_H_mean: float, T_H_std: float, T_L_mean: float, T_L_std: float,
    ...
    """
    Neue theta*-Arbeitsformel:

        100 theta_*^BM
        = 100 * (sigma(T_H)/mean(T_H))
              * (T_H/T_L)^(-a)
              * (sigma(T_L)/T_L)^(-b)
    """
...
def run_model(
    ...
    # 2) Globale High-Normierung über alle Fälle
    ...
    theta_star = theta_star_proxy_new(
        T_H_mean=T_H_mean, T_H_std=T_H_std,
        T_L_mean=T_L_mean, T_L_std=T_L_std,
        a=theta_a, b=theta_b
    )
    ...
    return {
        "params": {
            ...
            "high_norm": "global over all 15 cases",
        ...
        "theta_star_mean": float(theta_star),
        "theta_star_std": 0.0,  # derzeit globale Proxy-Definition
