...
static std::string prompt_str(const std::string& prompt, const std::string& default_val) {
    std::cout << prompt << " [" << default_val << "]: ";
    std::string line;
...
static bool prompt_bool(const std::string& prompt, bool default_val) {
    std::cout << prompt << " [" << (default_val ? "y" : "n") << "]: ";
    std::string line;
    if (!std::getline(std::cin, line) || line.empty())
        return default_val;
    if (line[0] == 'y' || line[0] == 'Y' || line[0] == '1')
        return true;
...
static uint32_t prompt_uint32(const std::string& prompt, uint32_t default_val) {
    std::cout << prompt << " [" << default_val << "]: ";
    std::string line;
    if (!std::getline(std::cin, line) || line.empty())
        return default_val;
...
static void run_interactive_dialog(int& trials, int& bits, int& B, std::string& score,
    bool& neighbor_safety, bool& fallback, bool& use_mr, bool& compare, uint32_t& master_seed) {
    std::cout << "\n--- Rabin/Hoff – interaktive Eingabe ---\n\n";
    trials = prompt_int("Anzahl Versuche (trials)", trials);
    bits = prompt_int("Bitlänge der Semiprimen (bits, max 64)", bits);
    B = prompt_int("Schranke B für Kandidaten", B);
    score = prompt_str("Score-Funktion (phase / norm / ...)", score);
    neighbor_safety = prompt_bool("Neighbor-Safety aktivieren?", neighbor_safety);
    fallback = prompt_bool("Fallback-Faktorisierung erlauben?", fallback);
    use_mr = prompt_bool("Miller-Rabin für Fallback (mr) an?", use_mr);
    compare = prompt_bool("Vergleichsmodus (MR on vs off)?", compare);
    master_seed = prompt_uint32("Zufallsseed (master_seed)", master_seed);
    std::cout << "\n";
}
...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        run_interactive_dialog(trials, bits, B, score, neighbor_safety, fallback, use_mr, compare, master_seed);
    } else {
