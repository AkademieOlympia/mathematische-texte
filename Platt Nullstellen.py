...
DEFAULT_ZETA_FOLDER = Path('/home/lmfdb/data/zeros/zeta/').expanduser()
SCRIPT_ZETA_FOLDER = Path(__file__).resolve().parent / 'zeta'


def _resolve_paths():
    """Resolve data and database paths with useful local fallbacks."""
    candidates = [DEFAULT_ZETA_FOLDER, SCRIPT_ZETA_FOLDER]
    for folder in candidates:
        data_path = folder / 'data'
        db_path = folder / 'index.db'
        if db_path.exists() and data_path.exists():
            return data_path, db_path
    # Keep first candidate as default, but fail later with a clear message.
    return candidates[0] / 'data', candidates[0] / 'index.db'


data_location, db_location = _resolve_paths()


def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description='List zeros from Platt zeta-zero tables.'
    )
    parser.add_argument('t', type=float, help='Start height t')
    parser.add_argument('count', type=int, help='Number of zeros to list')
...
if __name__ == "__main__":
    ...
    if not db_location.exists():
        raise FileNotFoundError(
            f'index.db not found at "{db_location}". '
            'Expected zeta data under "/home/lmfdb/data/zeros/zeta/" '
            'or next to this script in "./zeta/".'
        )
    if not data_location.exists():
        raise FileNotFoundError(
            f'data directory not found at "{data_location}".'
        )

    zeros = zeros_starting_at_t(args.t, args.count)
    for n, zero in zeros:
        print(f'{n}\t{zero}')
