...
### Universelle euklidische Hebung (Hurwitz-Kette, kein Beweispfad)

**Epistemik Schichten A/B/C/rot (PR #59):** `collatz_eabc_epistemik_schichten.md` — **A** hart (24I₃, Prime-Defekte, Fenster, Holonomie kombinatorisch), **B** geometrisch ($G_E$, Fluss, Harmonik), **C** ikonische Physik (Thomson, Schalen, Quaternionen — Interpretation), **rot** `RadiationSpace` (Lean-Scaffold, kein Theorem); parallele Spuren **Beweis $\|\$ Ikone**; Lean: `CollatzEabc/RadiationSpace.lean`. **Epistemik Physik vs. EABC (PR #59):** `collatz_eabc_epistemik_physik.md` — kanonische **drei Ebenen:** (1) Wegfunktion $T=\sum_j\ell_j^2/\gamma_{\mathrm{ref}}$ ohne Zeitdilatation (euklidisches Wegparadoxon, `collatz_eabc_kritische_abbildung.md` §7); (2) explizite Nicht-SRT-Abgrenzung (kein Zwillingsparadoxon, keine Eigenzeit); (3) Zirkulation/Holonomie $C_E=N_+-N_-$ als arithmetischer Kern — Sagnac/Umlaufsinn didaktisch **ja**, Relativität **nein**. Zentrale offene Frage: $\mathrm{Hol}_E=\lim(N_+-N_-)/(N_++N_-)$ — bevorzugter Umlaufsinn im Primfluss statt „wer altert langsamer?“. **Holonomie-Stufen (PR #59):** `collatz_eabc_holonomie_stufen.md` — Stufe 1 Analogie (erreicht), Stufe 2 echte Holonomie $\mathcal{P}\exp\oint A$ (offen), Stufe 3 Wilson-Loop $C_E=\sum\omega(\gamma)$ (diskret); Wachstumsfälle **A/B/C in $N$** (Fall A $D_E=O(1)$, Fall B $O(\sqrt{N})\Rightarrow\mathrm{Hol}_E\to 0$, Fall C $D_E\sim\alpha N\Rightarrow\mathrm{Hol}_E=\alpha$); Numerik `collatz_eabc_D_growth.py::classify_holonomy_growth`.

**EABC-Begriffshierarchie (PR #54):** `collatz_eabc_holonomie.md` — sechs Stufen: (1) $\kappa(p)$, (2) Signaturen $\sigma(Q)$, (3) Orientierung $\omega$, (4) Vierlings-Chiralität $\chi_E^{\mathrm{quad}}$ (**implementiert**); (5) Transport $T_n$, (6) Holonomie (**offen**). **Zyklus-Holonomie (PR #54):** `collatz_eabc_zyklus_holonomie.md` — $\chi_{\mathrm{Hol}}(N)$ auf Primfolge-Gleitfenstern; Transportebene in `collatz_eabc_transport.md`. **Zirkulationshypothese (PR #59, kanonisch):** `collatz_eabc_zirkulationshypothese.md` — EABC als Zirkulationstheorie (Shift $E(a,b)\to\oint_\gamma\alpha$), $N_\pm$, $C_E$, $D_E$, $S_E$, Hauptvermutung $S_E\to 0$, Fehlerterm-Hypothese (Prime Race); **Spektralgeometrie:** `collatz_eabc_zirkulation_spektral.md` — diskrete 1-Form $\alpha$, $\mathrm{Spec}(L_E)$; **Evolution & offene Analytik:** `collatz_eabc_evolution_analytik.md` — Bell$\to$Sagnac$\to C_E\to\mathrm{Spec}(L_E)$, $H_1(C_4)$, Wachstumsszenarien A–D ($X$) + Fall A/B/C ($N$), Dirichlet-Zerlegung mod $12$; Numerik: `collatz_eabc_D_growth.py`; **Fehlerterm-Vertiefung:** `collatz_eabc_fehlerterm_hypothese.md`; **Sagnac-Intuition:** `collatz_eabc_sagnac.md` (nur didaktisch); analytischer Beweisversuch: `collatz_eabc_holonomie_beweisversuch.md`; Numerik: `collatz_eabc_holonomie_fehlerterm.py` (`sagnac_report`), `collatz_eabc_sagnac_circulation.py` (`circulation_C_E`), `collatz_eabc_graph_laplacian.py` (`spectral_report`). **Kritische Abbildung (PR #59, Modellabbildung):** `collatz_eabc_kritische_abbildung.md` — Geschwindigkeitsmodell $s_v(x)=\tfrac12+\mathrm{i}v(x-\tfrac12)$, $x_{n,v}=\tfrac12+\gamma_n/v$, ABCEA/CEABC-Schaltkreise mit Lücken $(2,4,2,4)$; Numerik: `collatz_eabc_kritische_abbildung.py` (kein Physikanspruch). **Brachistochrone / Fermat (PR #59, Modell/Hypothese):** `collatz_eabc_brachistochrone.md` — fünf Potenzial-Kandidaten $V\in\{\ln x,\zeta\text{-Summe},D_E,K,-\log P\}$, $T=\sum\Delta s/v$, gerade vs. gestörte ABCEA-Pfade; Numerik: `collatz_eabc_brachistochrone.py` (Variationsprinzip, kein SRT). **Boxed Forschungsfragen:** Besitzt die Primfolge-induzierte EABC-Zirkulation einen nichttrivialen Fehlerterm? Welche Spektralinvarianten unterscheiden Prim- von Nichtprim-Normschalen? (`collatz_eabc_zirkulationshypothese.md` §8). **Bell–Holonomie (PR #54, sekundäre Analogie):** `collatz_eabc_bell_holonomie.md` — Taubenloch-Ungleichung als Zykluskonsistenz auf $G_E$ ($P_{\mathrm{same}}^{\mathrm{win}}\ge 1$ Theorem; marginale/CHSH-Lesarten experimentell); Numerik: `collatz_eabc_bell_inequality_test.py`. Numerik Übergangsgraph: `collatz_eabc_transition_graph.py`. **Wigner-Feld-Analogie (PR #59, Modell):** `collatz_eabc_wigner_analog.md` — signiertes Informationsfeld $W_{ab}$, $W_E=\#\mathrm{ABCE}-\#\mathrm{CEAB}$ (4-Pfad) vs. $D_E$ (5-Zyklus); Numerik: `collatz_eabc_wigner_field.py`. **Diskrete Geometrie (PR #59, kanonisch):** `collatz_eabc_diskrete_geometrie.md` — **primäre paper-ready Formulierung:** EABC = priminduzierter Fluss auf $C_4\cong S^1$ (nicht ABCE/CEAB-Zählstatistik); $G_E=(V,E)$, $E^+=\{EA,AB,BC,CE\}$, $E^-=\{EC,CB,BA,AE\}$; $\Phi_E=\lim W_E(X)=\lim C_E/S_E$; EABC-Vermutung $\langle\omega_E,h\rangle\neq 0$; drei Ebenen Zählung$\to$Fluss$\to$Kohomologie; Analogien AB/Sagnac/$L_{\mathrm{mag}}$; Numerik `Phi_E`/`flux_density_limit` in `collatz_eabc_hodge_eabc.py`. Detail: `collatz_eabc_uebergangsraum.md`, Zählgrößen: `collatz_eabc_zirkulationshypothese.md`. Aktueller Status: **ABCE/CEAB-Asymmetrie als Zirkulations-Fehlerterm/Bias** ($C_E=D_E$), nicht permanente Hauptterm-Holonomie.

Parallel zur operativen Brücke **L** (Collatz-Dynamik) formuliert `collatz_eabc_euklidische_hebung.md` einen **algebraischen Nebenzweig** über die Hurwitz-Divisionsalgebren $\mathbb{R}\subset\mathbb{C}\subset\mathbb{H}\subset\mathbb{O}$: Gitter $\Lambda_A$, Norm $N$, Projektion $\Pi_A$, Defekt $D_A(x)=x-\Pi_A(x)$ und Hebungsoperator $E_A(x,y)=(y,r)$ mit $z=xy^{-1}$, $q=\Pi_A(z)$, $r=x-qy$. **Kanonsiche Hypothese:** `collatz_eabc_normabstieg_hypothese.md` (EABC-Normabstiegs-Rahmen; testbare Gauß–EABC-Brücke §8: $p\equiv 1\pmod 4$ split $\leftrightarrow E\cup A$, $p\equiv 3\pmod 4$ inert $\leftrightarrow B\cup C$). **Spektralgeometrische Erzeugerhypothese (PR #54):** `collatz_eabc_quaternion_mass_hypothese.md` §12 — $\mathcal H=\bigsqcup_n\Sigma_n$ fundamental, $n\mapsto\mu_n\mapsto I(\mu_n)$, Anomalie $D(n)=I(\mu_n)-I_{\mathrm{ref}}(n)$; Primzahlen als mögliche Spektralanomalien (Emergenz, nicht Voraussetzung). **Plattenübergangs-Hypothese (PR #54):** `collatz_eabc_plattenuebergang.md` — $(\Sigma_n)_{n\ge 1}$ als diskrete Normschichtfolge auf $\Lambda_{\mathbb{O}}$; Prim = produkt-irreduzible Übergangsschicht (geometrisch trivial, EABC-neu erst via $Z(n)$, $\Delta Z(n)$ oder $D(n)$). Operative $Z$-Definitionen: `collatz_eabc_zerlegungsregimen.md`. **Experimente:** `collatz_eabc_Z_decomposition_test.py` ($Z_{\mathrm{EABC}}\approx Z_{\mathrm{fact}}$, $\omega$-Repackaging) und `collatz_eabc_shell_defekt_test.py` (rolling-$I_{\mathrm{ref}}$: kein Prim-$|D|$-Profil). **Oktanionische Erweiterung:** `collatz_eabc_oktonion_singularitaet.md` §3.5 — $\Sigma_n^{(8)}$ auf $S^7$, drei Konjekturen (Irreduzibilität / Isotropie / Anomalie); Quaternion-Daten ($n\le 200$) falsifizieren bisher Isotropie- und Anomalie-Richtung (rolling-$I_{\mathrm{ref}}$). **Assoziator-Spektralhypothese (PR #54):** `collatz_eabc_oktonion_assoziator_spektralhypothese.md` — $M_n(t)$, $M_n^E(t)$, $\hat D_E(s)$; Schlüssel: Nichtassoziativität + Normniveau + EABC (nicht Assoziator allein). **Experimente:** `collatz_eabc_oktonion_associator.py` ($\mathfrak{a}_E$), `collatz_eabc_oktonion_spectrum.py` (Histogramm + KL Prim vs. Composite). **Experiment:** `collatz_eabc_shell_defekt_test.py` vergleicht $I_{\mathrm{ref}}$-Varianten (rolling, $\omega$-, $\tau$-Stratum, $\mu_\infty$-Proxy). **Stub:** `collatz_eabc_oktonion_shell_stub.py` ($r_8(n)$ nur, $n\le 10$). **Etabliert:** $\mathbb{Z}$, $\mathbb{Z}[i]$, Hurwitz-$\mathbb{H}_{\mathrm H}$ euklidisch; $N$ multiplikativ auf allen vier Algebren; $\mathbb{O}$ verliert Assoziativität → klassischer $\gcd$/Ideal-Algorithmus **problematisch**; Split/inert in $\mathbb{Z}[i]$ = **Theorem**. **Forschungsfrage (nicht** „Euklid auf Oktanionen**"):** universeller **Norm-Defekt-Abstieg**; **Conjecture:** EABC-Primstruktur als minimale irreduzible Defektzustände — Verknüpfung zu `collatz_eabc_bernoulli_uebersetzung.md` §17 und `collatz_eabc_invarianzprogramm.md` (Fluktuationsfeld). **Experiment:** `collatz_eabc_euklid_hebung.py`, `collatz_eabc_gauss_defekt_test.py`, `collatz_eabc_shell_defekt_test.py`. **Holonomie/Transport (PR #54):** `collatz_eabc_holonomie.md` (Stufe 3–4 $\omega$, $\chi_E$); `collatz_eabc_transport.md` / `collatz_eabc_transition_graph.py` (Stufe 5 Kandidat $T$, $\chi_{\mathrm{trans}}$ vs. $\chi_E$). **Label:** Theorem / Definition / Forschungsfrage / Conjecture — **kein** Collatz-Beweisanspruch, **kein** ERPC.
