...
### EABC-Bernoulli-Zweig (arithmetischer Sensor, parallel)

Parallel zu Morley und $\kappa$ beschreibt `collatz_eabc_bernoulli_uebersetzung.md` die **EABC-Deutung** von-Staudt-Clausen-Signaturen: die Filtration $\{B_{2n}\}\leftrightarrow\{p:p-1\mid 2n\}$ wird über den nativen mod-$12$-Vektor $V(B_{2n})=(e_n,a_n,b_n,c_n)$ und den Sensor $\Phi_B$ ausgewertet (`collatz_eabc_bernoulli_sensor.py` $\to$ `collatz_eabc_bernoulli_sensor.json`). Chirale Observablen $\sigma_B$, $\chi_B$, $\iota_{\mathrm{chir}}$ sind **Experiment** (Test 1); Zeta-Kopplung und Primzahl-Krümmung bleiben **zukünftig**. *ERPC* bezeichnet hier dieselbe EABC-Struktur — kein separates Projekt. Bernoulli-Lyapunov bleibt **No-Go**; dieser Zweig ersetzt weder Morley noch $\mathcal{L}_{\mathrm{arith}}$.
