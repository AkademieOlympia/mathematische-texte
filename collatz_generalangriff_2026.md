...
**EABC-Begriffshierarchie (PR #54):** `collatz_eabc_holonomie.md` — sechs Stufen: (1) $\kappa(p)$, (2) Signaturen $\sigma(Q)$, (3) Orientierung $\omega$, (4) Vierlings-Chiralität $\chi_E^{\mathrm{quad}}$ (**implementiert**); (5) Transport $T_n$, (6) Holonomie (**offen**). **Zyklus-Holonomie (PR #54):** `collatz_eabc_zyklus_holonomie.md` — $\chi_{\mathrm{Hol}}(N)$ auf Primfolge-Gleitfenstern; Transportebene in `collatz_eabc_transport.md`. **Endform Fehlerterm (PR #54):** `collatz_eabc_fehlerterm_hypothese.md` — $N_\pm$, Hauptvermutung $\mathrm{Hol}_E=0$, Fehlerterm-Hypothese ($L$-Funktionen mod $12$), $\widetilde{D}_E$; analytischer Beweisversuch: `collatz_eabc_holonomie_beweisversuch.md`; Numerik: `collatz_eabc_holonomie_fehlerterm.py`. **Bell–Holonomie-Brücke (PR #54):** `collatz_eabc_bell_holonomie.md` — Taubenloch-Ungleichung als Zykluskonsistenz auf $G_E$ ($P_{\mathrm{same}}^{\mathrm{win}}\ge 1$ Theorem; marginale/CHSH-Lesarten experimentell); Numerik: `collatz_eabc_bell_inequality_test.py`. Numerik Übergangsgraph: `collatz_eabc_transition_graph.py`. Aktueller Status: **ABCE/CEAB-Asymmetrie als Fehlerterm/Bias**, nicht permanente Hauptterm-Holonomie.
