...
Parallel zur operativen Brücke **L** (Collatz-Dynamik) formuliert `collatz_eabc_euklidische_hebung.md` einen **algebraischen Nebenzweig** über die Hurwitz-Divisionsalgebren $\mathbb{R}\subset\mathbb{C}\subset\mathbb{H}\subset\mathbb{O}$: Gitter $\Lambda_A$, Norm $N$, Projektion $\Pi_A$, Defekt $D_A(x)=x-\Pi_A(x)$ und Hebungsoperator $E_A(x,y)=(y,r)$ mit $z=xy^{-1}$, $q=\Pi_A(z)$, $r=x-qy$. **Kanonsiche Hypothese:** `collatz_eabc_normabstieg_hypothese.md` (EABC-Normabstiegs-Rahmen; testbare Gauß–EABC-Brücke §8: $p\equiv 1\pmod 4$ split $\leftrightarrow E\cup A$, $p\equiv 3\pmod 4$ inert $\leftrightarrow B\cup C$). **Spektralgeometrische Hauptvermutung (PR #54):** `collatz_eabc_quaternion_mass_hypothese.md` §12 — $\mathcal H=\bigsqcup_n\Sigma_n$ fundamental, $n\mapsto\mu_n\mapsto I(\mu_n)$, Anomalie $D(n)=I(\mu_n)-I_{\mathrm{ref}}(n)$; Primzahlen als mögliche Spektralanomalien (Emergenz, nicht Voraussetzung). **Experiment:** `collatz_eabc_shell_defekt_test.py` vergleicht $I_{\mathrm{ref}}$-Varianten (rolling, $\omega$-, $\tau$-Stratum, $\mu_\infty$-Proxy). **Etabliert:** $\mathbb{Z}$, $\mathbb{Z}[i]$, Hurwitz-$\mathbb{H}_{\mathrm H}$ euklidisch; $N$ multiplikativ auf allen vier Algebren; $\mathbb{O}$ verliert Assoziativität → klassischer $\gcd$/Ideal-Algorithmus **problematisch**; Split/inert in $\mathbb{Z}[i]$ = **Theorem**. **Forschungsfrage (nicht** „Euklid auf Oktanionen**"):** universeller **Norm-Defekt-Abstieg**; **Conjecture:** EABC-Primstruktur als minimale irreduzible Defektzustände — Verknüpfung zu `collatz_eabc_bernoulli_uebersetzung.md` §17 und `collatz_eabc_invarianzprogramm.md` (Fluktuationsfeld). **Experimente:** `collatz_eabc_euklid_hebung.py`, `collatz_eabc_gauss_defekt_test.py`, `collatz_eabc_shell_defekt_test.py`. **Label:** Theorem / Definition / Forschungsfrage / Conjecture — **kein** Collatz-Beweisanspruch, **kein** ERPC.
