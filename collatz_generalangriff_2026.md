...
**EABC-Begriffshierarchie (PR #54):** `collatz_eabc_holonomie.md` — sechs Stufen: (1) $\kappa(p)$, (2) Signaturen $\sigma(Q)$, (3) Orientierung $\omega$, (4) Vierlings-Chiralität $\chi_E^{\mathrm{quad}}$ (**implementiert**); (5) Transport $T_n$, (6) Holonomie (**offen**). **Zyklus-Holonomie-Hypothese (PR #54):** `collatz_eabc_zyklus_holonomie.md` — $\chi_E(N)=\sum\Omega/\#\{\Omega\neq 0\}$ auf Primfolge-Gleitfenstern, $\mathrm{Hol}_E=\lim\chi_E(N)$; Transportebene in `collatz_eabc_transport.md`. **Analytischer Beweisversuch (PR #54):** `collatz_eabc_holonomie_beweisversuch.md` — mod-$12$-Symmetrie, Lückenmuster $(2,4,2,4)$, konservative Lesart $\mathrm{Hol}_E=0$ mit Fehlerterm $D_E$ (Chebyshev-Analogie); Numerik: `collatz_eabc_holonomie_fehlerterm.py`. Numerik Übergangsgraph: `collatz_eabc_transition_graph.py`. Aktueller Status: **Orientierungsasymmetrie auf Übergängen**, nicht wahre DG-Holonomie.
