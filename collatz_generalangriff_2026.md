...
Parallel zu Morley und κ formuliert `collatz_eabc_invarianzprogramm.md` das **EABC-Invarianzprogramm** in Tao-strenger Form: **Definition** $\kappa:P_{>3}\to\{E,A,B,C\}$ ($E\equiv 1$, $A\equiv 5$, $B\equiv 7$, $C\equiv 11$ mod $12$), Zählvektor $V(x)$, Simplex-Anteil $S(x)\in\Delta_3$, EABC-Invariante $\mathcal{I}$ (Grenzwert oder asymptotisch beschränkte Schwankungen), Beispiel $\chi(x)=((E+C)-(A+B))/\pi_{>3}(x)$, Prim-Vierling-Signaturen $\sigma(Q)\in\Sigma_4$. **Forschungsfragen:** limitierende Verteilung $\mu$ auf $\Sigma_4$; existiert $\mathcal{I}(S(x))=c+o(1)$? **Arbeitshypothese (Conjecture):** nicht-triviale EABC-Invarianten, nicht rekonstruierbar aus $\pi(x)$ allein — **ohne** Behauptung, EABC erzeuge Primzahlen oder $8$D-Projektion. Philosophischer Kontext: `collatz_eabc_bernoulli_uebersetzung.md` §22 (wenn vorhanden). **Experiment:** `collatz_eabc_invarianzprogramm.py` $\to$ `collatz_eabc_invarianzprogramm.json`. **Label:** Definition / Forschungsfrage / Conjecture / Experiment.
