...
Parallel zur operativen Brücke **L** (Collatz-Dynamik) formuliert `collatz_eabc_euklidische_hebung.md` einen **algebraischen Nebenzweig** über die Hurwitz-Divisionsalgebren $\mathbb{R}\subset\mathbb{C}\subset\mathbb{H}\subset\mathbb{O}$: Gitter $\Lambda_A$, Norm $N$, Projektion $\Pi_A$, Defekt $D_A(x)=x-\Pi_A(x)$ und Hebungsoperator $E_A(x,y)=(y,r)$ mit $z=xy^{-1}$, $q=\Pi_A(z)$, $r=x-qy$. **Etabliert:** $\mathbb{Z}$, $\mathbb{Z}[i]$, Hurwitz-$\mathbb{H}_{\mathrm H}$ euklidisch; $N$ multiplikativ auf allen vier Algebren; $\mathbb{O}$ verliert Assoziativität → klassischer $\gcd$/Ideal-Algorithmus **problematisch**. **Forschungsfrage (nicht** „Euklid auf Oktanionen**"):** universeller **Norm-Defekt-Abstieg**; **Conjecture:** EABC-Primstruktur als minimale irreduzible Defektzustände in der $\mathbb{R}$-Spezialisierung — Verknüpfung zu `collatz_eabc_bernoulli_uebersetzung.md` §17 und `collatz_eabc_invarianzprogramm.md` (Fluktuationsfeld). **Experiment:** `collatz_eabc_euklid_hebung.py`. **Label:** Theorem / Definition / Forschungsfrage / Conjecture — **kein** Collatz-Beweisanspruch, **kein** ERPC.
