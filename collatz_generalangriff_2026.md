...
**EABC-Holographiehypothese (§19, nur in `collatz_eabc_bernoulli_uebersetzung.md`):** Holographisches **Leitbild** (Leonard Susskind; Juan Maldacena AdS/CFT 1997 — **nur Analogie**, keine Identität „EABC = AdS/CFT"): Bulk = 8D-Tetraederfüllung (§17), Boundary = EABC-Randdaten mod $12$ (§18), Spektrum = Prim-/Zeta-/Bernoulli-Signaturen (§2). **Fünfersprünge** = Projektbegriff für Fibonacci-skalierte Kollapsmarker (Vierling$\to$Fünfling-Vorlage; `Vier Fünf Synchronisation.py`; Definition teils **noch zu präzisieren**). Boxed Kette: Peano-Achse $\leftarrow$ EABC-Randdaten $\leftarrow$ 8D-Tetraederfüllung. **Label:** Conjecture / Forschungsvision — kein physikalischer oder arithmetischer Beweis.

**Drei Wachstumsprojektionen und Fünfstufenprogramm (§20, nur in `collatz_eabc_bernoulli_uebersetzung.md`):** Peano ($n\mapsto n+1$, lineare Schichten), Fibonacci ($F_{k+1}=F_k+F_{k-1}$, $\varphi$ als dominante Skala), EABC ($X_{n+1}=T(X_n)$, vier Kanäle) als **drei Projektionen desselben Füllprozesses**. Fünf Stufen: (1) Peano-lineare Dynamik, (2) Fibonacci-Gedächtnis, (3) EABC-Rekonfiguration mit Prim als kritischen Defekten, (4) Fünfersprünge als pentagonale Defektstörung reiner Tetraederdynamik ($x^2=x+1$, Ikosaeder/Dodekaeder), (5) holographische Lesart $F\to B\to\mathbb{N}$ (Füllraum $\to$ EABC-Randcode $\to$ Peano). Boxed **EABC-Holographie-Vermutung:** höherdimensionaler diskreter Füllraum; Primzahlen = Randprojektion kritischer Defekte; Fibonacci-Skalen = Rekonfigurationen mit pentagonalen Defekten; nichttriviale Zeta-Nullstellen = Spektrum der Defektdynamik. **Experiment (optional):** `collatz_eabc_fibonacci_reconfig_test.py` scannt $n$ nahe $F_k$ auf Auffälligkeiten in $V_n$, $\sigma$, $\chi$, $\iota_{\mathrm{chir}}$, Quadrupelstrukturen. **Label:** Conjecture / Experiment — Susskind/Maldacena nur Analogie, keine Identität.

**EABC-Epistemik: Primzahlen, Fossilien und Invarianten (§21, nur in `collatz_eabc_bernoulli_uebersetzung.md`):** Präzisiert die Tao-Trennung **Definition** vs. **Conjecture/Forschungsvision** für die Primzahl-Lesart. **Definition (nicht verhandelbar):** $p$ prim $\Longleftrightarrow p>1$ und genau zwei positive Teiler — Primzahlen sind **keine** hypothetischen Physikobjekte wie Elektronen. **Conjecture/Forschungsvision:** Primzahlen können wohldefinierte mathematische Objekte sein und gleichwohl **keine fundamentalen Objekte der Theorie** — sichtbare „Knochen", während die eigentlichen „Fossilien" statistische Invarianten ($\sigma$, $\chi$, $\iota_{\mathrm{chir}}$, $\Delta Q_4$, $V_n$, Quadrupelzeuge in Sensor/Lean-Tests) über große Primdynamiken sind. Erfolgskriterium: stabiles $\mathcal{I}(E,A,B,C)$, **nicht** eine neue $p_n$-Formel. Physik-Analogien (Planeten$\to$Newton, Spektrallinien$\to$QM, Teilchen$\to$QFT) nur **Heuristik**. Verknüpfung mit §1 (Zerlegungsprinzip), §2 (Zustandsraum), §20 (Fünfstufenprogramm). Riemann, Weyl, Connes, Penrose als **offene** strukturelle Querverweise — ob EABC diese Organisation trifft, **offen**.

**EABC-Invarianz-Hypothese: Primzahlen als Messreihe (§22, nur in `collatz_eabc_bernoulli_uebersetzung.md`):** Verfeinert §21 philosophisch: Primzahlen weder bloße „Realität" noch nur Fossilien, sondern **Messreihe** im physikalischen Sinn — beobachtbare diskrete Folge $2,3,5,7,\ldots$ als möglicher **Output** tieferer Struktur (Balmer-Linien-Heuristik vor QM). **EABC-Programmverschiebung:** Invarianten ($\sigma$, $\chi$, $\iota_{\mathrm{chir}}$, Quadrupelstrukturen, $V_n$, Zeta-Spektren) **fundamental**; Einzelprimzahlen **sekundär**. Klein-Erlangen-Heuristik: Geometrie = Invarianten unter Transformationen; Kernfrage „welche Größen persistieren?" statt „welche Primzahl als Nächstes?". $(E,A,B,C)$ als Klein-$V_4$-Symmetrieträger (§18), nicht nur Klassensammlung. Boxed **EABC-Invarianz-Hypothese** (**Label:** Conjecture, vorsichtig): EABC-Klassen = beobachtbare Projektion, nicht die Struktur selbst — behauptet **nicht**, EABC habe bereits die wahre Geometrie gefunden. Morley-Zweig **parallel** (vgl. Morley-Sensorik oben), **nicht** fusionieren. Querverweise: §1, §18, §21; Riemann, Klein, Connes — **offen**.
