...
lemma lemma13 (n : ℕ) (ε : ℝ) (hn : n ≥ 1) (hε : ε > 0) (hε_small : ε < 1/100)  : (tau n : ℝ) / ((n : ℝ) ^ ε) ≤ (2 / ε : ℝ) ^ ((2 : ℝ) ^ (1 / ε)) := by
  ...
  have h_large : ∀ p ∈ n.primeFactors.filter (fun (p : ℕ) => ¬((p : ℝ) ^ ε < 2)),
    ...
    rw [Finset.mem_filter] at hp
    -- p^ε ≥ 2
    have hp_ge : 2 ≤ (p : ℝ) ^ ε := not_lt.mp hp.2
...
lemma lemma29 (ε : ℝ) (N r : ℕ) (hε : ε > 0) (hε_small : ε < 1/100)  (hN : N ≥ 1) (hr : r ≥ 1) :
  Finset.sum (radical_set r N) (fun n => (1 : ℝ) / ((n : ℝ) ^ ε)) ≥
  ...
  have sum_const :
      Finset.sum (radical_set r N) (fun _ => (1 : ℝ) / ((N : ℝ) ^ ε)) =
        (radical_set r N).card * (1 / ((N : ℝ) ^ ε)) := by
    rw [Finset.sum_const]
    simp [nsmul_eq_mul]
  ...
  have h_sum_ge :
      Finset.sum (radical_set r N) (fun n => (1 : ℝ) / ((n : ℝ) ^ ε)) ≥
        Finset.sum (radical_set r N) (fun _ => (1 : ℝ) / ((N : ℝ) ^ ε)) := by
    apply Finset.sum_le_sum
    exact h_each_term

  rw [sum_const] at h_sum_ge
...
theorem geometric_series (x : ℝ) (hx_pos : 0 < x) (hx_lt_one : x < 1) : ∑' (a : ℕ), x ^ (a + 1) = x / (1 - x) := by
  ...
  have h_geom : ∑' (n : ℕ), x ^ n = (1 - x)⁻¹ := tsum_geometric_of_lt_one hx_nonneg hx_lt_one
  calc
    ∑' (a : ℕ), x ^ (a + 1)
      = ∑' (a : ℕ), x * x ^ a := by
          congr with a
...
lemma lemma36 (ε : ℝ) (s : Finset ℕ) (hε : ε > 0) (hε_small : ε < 1/100)  (hs_prime : ∀ p ∈ s, p.Prime) :
  Finset.prod s (fun p => ∑' (a : ℕ), (1 : ℝ) / ((p : ℝ) ^ ((a + 1) * ε))) =
    Finset.prod s (fun p => (1 : ℝ) / ((p : ℝ) ^ ε - 1)) := by
...
lemma lemma37 (ε : ℝ) (s : Finset ℕ) (hε : ε > 0) (hε_small : ε < 1/100)  (hs_prime : ∀ p ∈ s, p.Prime) :
  Finset.prod s (fun p => (1 : ℝ) / ((p : ℝ) ^ ε - 1)) =
    Finset.prod (s.filter (fun (p : ℕ) => (p : ℝ) ^ ε ≥ 2))
      (fun p => (1 : ℝ) / ((p : ℝ) ^ ε - 1)) *
    Finset.prod (s.filter (fun (p : ℕ) => (p : ℝ) ^ ε < 2))
      (fun p => (1 : ℝ) / ((p : ℝ) ^ ε - 1)) := by
...
lemma map_to_exponents_injective (s : Finset ℕ) (hs_prime : ∀ p ∈ s, p.Prime) :
  ...
  have h_all_primes : ∀ p : ℕ, n₁.factorization p = n₂.factorization p := by
    ...
    · -- For p not in s, we need to show factorization is 0
      ...
      have hp₁_zero : n₁.factorization p = 0 := by
        rw [Finsupp.mem_support_iff] at hp₁
        simpa using hp₁
      have hp₂_zero : n₂.factorization p = 0 := by
        rw [Finsupp.mem_support_iff] at hp₂
        simpa using hp₂
      rw [hp₁_zero, hp₂_zero]
...
lemma finset_sum_le_tsum_of_subset (s : Finset ℕ) (t : Set ℕ) (f : ℕ → ℝ)
  ...
  have h_le : Finset.sum s f ≤ ∑' n : ℕ, t.indicator f n := by
    -- First show ∑ n in s, f n = ∑ n in s, t.indicator f n
    have h_eq : Finset.sum s f = Finset.sum s (t.indicator f) := by
...
lemma lemma31 (ε : ℝ) (N : ℕ) (r : ℕ) (hε : ε > 0) (hε_small : ε < 1/100)  (hN : N ≥ 1) (hr : r ≥ 1) (hr_sf : rad r = r) :
  Finset.sum (radical_set r N) (fun n => (1 : ℝ) / ((n : ℝ) ^ ε)) ≤
    ∑' (n : target_set r.primeFactors), (1 : ℝ) / ((n : ℝ) ^ ε) := by
